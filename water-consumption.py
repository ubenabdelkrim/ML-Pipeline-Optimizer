...
parser = argparse.ArgumentParser(description='Script for geospatial processing using Lithops.')
parser.add_argument('--splits', type=int, default=3, help='Number of splits for processing.')
parser.add_argument('--runtime_memory', type=int, default=2048, help='Memory allocated for each runtime (in MB).')
parser.add_argument('--ephemeral_storage', type=int, default=512, help='Ephemeral storage allocated for each runtime (in MB).')
...
keys = storage.list_keys(DATA_BUCKET)

# Verificar si hay objetos en el bucket
if keys:
    # Eliminar todos los objetos en el bucket
    storage.delete_objects(DATA_BUCKET, keys)
    print(f"Todo el contenido del bucket '{DATA_BUCKET}' ha sido eliminado.")
else:
    print(f"No se encontraron objetos en el bucket '{DATA_BUCKET}'.")
...
print(f"Executor ID: {fexec.executor_id}")
print(f"Runtime memory: {fexec.config['aws_lambda']['runtime_memory']} MB")
print(f"Ephemeral storage: {fexec.config['aws_lambda']['ephemeral_storage']} MB")
print(f"Worker processes: {fexec.config['aws_lambda']['worker_processes']}")
print(f"Invoke pool threads: {fexec.config['aws_lambda']['invoke_pool_threads']}")
...
for input_key in files:
    if not input_key.endswith('/'):
        ...
        print(f"Skipping directory-like key: {input_key}")
...
filename = f"exec_ids_{args.splits}_splits_v4.csv"
...
vcpus = round(runtime_memory / 1769, 2)  # Calculate number of vCPUs
